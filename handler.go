...
func Post(w http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("Secret") == os.Getenv("SECRET") {
		if msg.Message == "loc" || msg.Message == "Loc" || msg.Message == "lokasi" || msg.LiveLoc {
			...
			randm := []string{
				"Hallo " + msg.Alias_name + "\n maaf kita sedang sibuk \n mohon tunggu \n nggaa lama kok \n",
				"mohon jangan spam",
				"untuk pemesanan kita belum tersedia mohon maaf",
				"kita akan memperbaiki secepat mungkin, mohon ditunggu",
...
func Liveloc(w http.ResponseWriter, r *http.Request) {
	...
	reply := fmt.Sprintf("Aku sedang ramal kamu pasti berada di %s \n Koordinatenya : %s - %s\n", location,
